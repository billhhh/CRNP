...
def get_arguments():
    ...
    """
    parser = argparse.ArgumentParser(description="RNP model for 3D Medical Image Segmentation.")

    parser.add_argument("--data_dir", type=str, default='./datalist/')
    parser.add_argument("--train_list", type=str, default='BraTS20_train.csv')
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        model = CRNP(args=args, norm_cfg=args.norm_cfg, activation_cfg=args.activation_cfg,
                     num_classes=args.num_classes, weight_std=args.weight_std, self_att=True)
        model.train()
        ...
        for i_iter, batch in enumerate(trainloader):
            ...
            print('iter = {} of {} completed, lr = {:.4}, seg_loss = {:.4}'.format(
                    i_iter, args.num_steps, lr, (term_seg_Dice+term_seg_BCE).cpu().data.numpy()))
